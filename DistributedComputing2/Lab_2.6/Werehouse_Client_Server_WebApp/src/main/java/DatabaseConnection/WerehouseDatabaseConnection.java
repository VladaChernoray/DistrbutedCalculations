...
public class WerehouseDatabaseConnection {
    ...
    public static ArrayList<Product> getProductsBySectionID(int groupID) {
        try {
            Class.forName(jdbcDriverName);
            Connection connection = DriverManager.getConnection(url, user, password);
            String sqlQuery = "SELECT * FROM werehouse.products " +
    ...
    public static final boolean deleteSectionByID(int id) {
        try {
            ...
            stmt.setInt(1, id);
            stmt.execute();
            sqlQuery = "DELETE FROM werehouse.products WHERE sectionID=?;";
    ...
    public static final boolean deleteSectionByName(String name) {
        ...
        if (section == null) {
            ...
            try {
                ...
                stmt = connection.prepareStatement(sqlQuery);
                stmt.setInt(1, section.getGroupID());
                stmt.execute();
                connection.close();
                return true;
            } catch (Exception e) {
    ...
    public static final boolean changeSectionForProduct(int newSectionID, int productID) {
        Product product = getSectionByID(newSectionID);
        if (product == null) {
            return false;
        } else {
            try {
