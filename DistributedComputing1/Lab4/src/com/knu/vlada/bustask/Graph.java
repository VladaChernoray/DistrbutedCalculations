...
public class Graph {
    ...
    private Cell findCities(String cityFrom, String cityTo) {
        ...
        for (int i = 0; i < cities.size(); i++) {
            if (cities.get(i).equals(cityFrom)) cell.from = i;
            if(cities.get(i).equals(cityTo)) cell.to = i;
            if(cell.from != -1 && cell.to != -1) break;
        }
        return cell;
    }
    ...
    public void addCities(String city) {
        writeLock.lock();
        cities.add(city);
        for (ArrayList<Integer> integers : graph) integers.add(0);
        ArrayList<Integer> list = new ArrayList<>(cities.size());
        for(int i = 0; i < cities.size(); i++) list.add(0);
        graph.add(list);
        writeLock.unlock();
    }
    ...
    public void printGraph() {
        ...
        for (ArrayList<Integer> integers : graph) System.out.println(integers);
        readLock.unlock();
    }
